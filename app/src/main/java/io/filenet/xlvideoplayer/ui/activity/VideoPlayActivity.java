...
public class VideoPlayActivity extends BasicActivity implements View.OnClickListener, VideoPlayer.OnVideoPlayerStateListener
    ...
    private List<ModuleInfo> mModuleInfoList = new ArrayList<>();
            ...
            switch (msg.what){
                case LAODMORE:
                    runOnUiThread(new Runnable() {
                        ...
                        public void run() {
                            if (isloading) return;
                            mAdapter.update(getVideoInfoList(mModuleInfoList.size()/18));
...
//                }
    ...
    private synchronized List<ModuleInfo> getVideoInfoList(int index){
        mHandle.sendEmptyMessage(SHOWLOAD);
//        for (int i=0+index*18; i<18+index*18; i++){
//            if (i%3 == 0) mModuleInfoList.add(new ModuleInfo("这是一个很长很长很长的标题 ---- "+i,R.mipmap.img_2,mUrls[2]));
//            if (i%3 == 1) mModuleInfoList.add(new ModuleInfo("这是一个很长很长很长的标题 ---- "+i,R.mipmap.img_3,mUrls[1]));
//            if (i%3 == 2) mModuleInfoList.add(new ModuleInfo("这是一个很长很长很长的标题 ---- "+i,R.mipmap.img_4,mUrls[0]));
//        }
